...
class ESACCIPreprocessor(BasePreProcessor):
    ...
    @staticmethod
    def _reassign_lc_class_to_groups(
        ds: xr.Dataset, legend: pd.DataFrame
    ) -> xr.Dataset:
        ...
        mapping = dict(mapping=dict(zip(list(legend.code), list(legend.group_value))))
        ds["lc_class_group"] = xr.apply_ufunc(remap, ds, kwargs=mapping).lc_class

        return ds

    def _one_hot_encode(
        self, ds: xr.Dataset, group: bool = True
        ...
        if group:
            for grouped_val in legend.group_value.unique():
                group_values = list(legend.loc[legend.group_value == grouped_val].code)
                group_label = str(
            ...
            for idx, row in legend.iterrows():
                ...
                ds[f"{label}_one_hot"] = ds.lc_class.where(
                    ds.lc_class == value, 0
                ).clip(min=0, max=1)
    ...
    def _preprocess_interim_files(
        ...
        if one_hot_encode:
            ...
            lc_class_ds = self._reassign_lc_class_to_groups(
                ds.lc_class.to_dataset(name="lc_class"), legend_df
            )
    ...
    def preprocess(
        ...
        self._preprocess_interim_files(
            subset_str=subset_str, one_hot_encode=one_hot_encode, group=group
        )
