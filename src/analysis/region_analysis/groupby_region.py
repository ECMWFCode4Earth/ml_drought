...
class GroupbyRegion:
    ...
    def analyze(self, da: xr.DataArray,
        ...
        region_da, region_lookup, region_group_name = self.load_region_data(self.region_data_path)
    ...
    def get_values_for_region(da: xr.DataArray,
                              ...
                              region_lookup: Dict,
                              admin_level_name: str) -> pd.DataFrame:
        ...
        for valid_region_id in valid_region_ids:
            for time in da.time.values:
                ...
                values = drop_nans_and_flatten(
                    da.sel(time=time)
                    .where(region_da == valid_region_id)
                )
    ...
    @staticmethod
    def calculate_mean_per_region(da: xr.DataArray,
                                  region_da: xr.DataArray,
                                  region_lookup: Dict,
                                  admin_level_name: str) -> pd.DataFrame:
...
class KenyaGroupbyRegion(GroupbyRegion):
    ...
    def __init__(self, data_dir: Path = Path('data'),) -> None:
        super().__init__(
            data_dir=data_dir, country='kenya'
        )
    ...
    def get_admin_level(self, selection: str) -> AdminBoundaries:
        level_1 = AdminBoundaries(
            ...
            nc_filepath=(
                self.data_dir / 'analysis' / 'boundaries_preprocessed' /
        ...
        level_3 = AdminBoundaries(
            ...
            nc_filepath=(
                self.data_dir / 'analysis' / 'boundaries_preprocessed' / 'division_l3_kenya.nc'
            ),
        )
        ...
        level_3_wards = AdminBoundaries(
            ...
            nc_filepath=(
                self.data_dir / 'analysis' / 'boundaries_preprocessed' / 'level_3_wards_kenya.nc'
            ),
        )
        ...
        level_4 = AdminBoundaries(
            ...
            nc_filepath=(
                self.data_dir / 'analysis' / 'boundaries_preprocessed' / 'location_l4_kenya.nc'
            ),
        )
