...
class NNBase(ModelBase):
    ...
    def explain(self, x: Any) -> Union[np.ndarray, Tuple[np.ndarray, ...]]:
    ...
    def _get_background(self, sample_size: int = 100) -> List[torch.Tensor]:
    ...
    def _one_hot_months(
        self, indices: Optional[torch.Tensor]
    ) -> Optional[torch.Tensor]:
        if self.include_pred_month:
            assert (
                indices is not None
            ), f"Years can't be None if include pred months is True"
