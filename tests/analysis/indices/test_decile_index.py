...
class TestDecileIndex:
    ...
    def test_initialisation(self, tmp_path):
        ...
        assert di.name == 'decile_index', f"Expected name"\
    ...
    def test_fit(self, tmp_path):
        ...
        assert "quintile" in vars_, f"Expecting `quintile` variable in"\
            "`self.index`"

        vals = np.unique(di.index.quintile.values)
        assert all(np.isin(np.arange(1, 6), vals)), f"Expect quintiles to"\
            f" be from 1 - 5. Got: {vals}"

        assert di.index.rank_norm.min().values == 0, f"Expect minmum "\
            f"rank_norm to be 0. Got: {di.index.rank_norm.min().values}"

        assert di.index.rank_norm.max().values == 100, f"Expect max "\
