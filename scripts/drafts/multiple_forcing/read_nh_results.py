...
4) Extract Results
    `cd /home/tommy/ml_drought; ipython --pdb scripts/drafts/multiple_forcing/read_nh_results.py -- --run_dir /cats/datastore/data/runs/complexity --ensemble True --ensemble_filename /cats/datastore/data/runs/complexity/ensemble_results.p`
•) Read each individual member
    ipython -c "from scripts.drafts.io_results import read_ensemble_member_results; from pathlib import Path; ds = read_ensemble_member_results(Path('/cats/datastore/data/runs/complexity')); ds.to_netcdf('/cats/datastore/data/runs/complexity/all_results.nc')"
