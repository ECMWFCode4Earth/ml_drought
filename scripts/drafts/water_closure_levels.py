...
def create_closure_masks(
    ...
    assert all(
        [(c <= 1) & (c > 0) for c in closure_thresholds]
    ), "Need to be between 0 and 1 (ratio)"
    #  assign water year as a coordinate
    ds_wy = ds.assign_coords(
        wy=("time", [assign_wateryear(dt) for dt in ds.time.values])
    )
    ...
    ann_closure = (
        ann_aggregated["precipitation"]
        - ann_aggregated["pet"]
        - ann_aggregated["discharge_spec"]
    )
    ...
    for c in closure_thresholds:
        ...
        bool_wb = (
            bool_wb.to_dataset(**dict(name="mask"))
            .assign_coords(threshold=c)
            .expand_dims(dim="threshold")
        )
...
def get_condition_sids(thresholds: xr.Dataset, threshold_level: float):
    assert (
        threshold_level in thresholds.threshold.values
    ), f"Threshold level must be in: {thresholds.threshold.values}"
    condition_sids = thresholds.where(
        thresholds.sel(threshold=threshold_level)["mask"], drop=True
    ).station_id.values
