...
for ix, station_id in tqdm(enumerate(lstm_preds.station_id.values, desc="Calculating Metrics")):
    # station_id = lstm_preds.station_id.values[0]
    df = lstm_preds.sel(station_id=station_id).to_dataframe().drop(
        "station_id", axis=1)
    ...
    result_df = hs.make_table(merged_dataframe=df,
                              ...
                              seasonal_periods=[
                                  ['12-01', '02-29'], ['03-01', '05-31'], ['06-01', '08-31'], ['09-01', '11-30']],
    ...
    result_df = pd.concat([result_df, inv_kge_df.drop(
        "Location", axis=1), log_nse_df.drop("Location", axis=1)], axis=1)
